...
{
    static const std::string testValue = R"(
    ...
    ON_CALL(handler, lineStart(_)).WillByDefault(Return(true));
    ON_CALL(handler, cell(_, _, _)).WillByDefault(Return(true));

    EXPECT_CALL(handler, fileStart()).Times(1);
    EXPECT_CALL(handler, fileEnd()).Times(1);
    EXPECT_CALL(handler, lineStart(Gt(0))).Times(2);
    EXPECT_CALL(handler, lineEnd(Gt(0))).Times(2);
    EXPECT_CALL(handler, cell(Gt(0), _, _)).Times(10);

    CsvFileReader::read(testValue, handler);
}
